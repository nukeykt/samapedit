...
unsigned int dbReadMapCRC(const char *pPath)
{
    ...
    if (!pNode)
    {
        buildprintf("Error opening map file %s", pPath);
...
// #endif
    ...
    if (memcmp(header.signature, "BLM\x1a", 4))
    {
        buildprintf("Map file corrupted");
        gSysRes.Unlock(pNode);
        return -1;
    }
    byte_1A76C8 = 0;
    if ((header.version & 0xff00) == 0x700) {
        ...
        buildprintf("Map file is wrong version");
        gSysRes.Unlock(pNode);
        return -1;
    }
    ...
    *pSector = mapHeader.ate;
    ...
    if (byte_1A76C8)
    {
        ...
        {
            buildprintf("Corrupted Map file");
            gSysRes.Unlock(pNode);
            return -1;
        }
    }
